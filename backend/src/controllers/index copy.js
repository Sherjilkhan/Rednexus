...
export const attendCamp = wrap(async (req, res) =>
  res.json({ attendance: await camps.markAttendance(req.params.id, req.user.donor_id, req.body.attending !== false) }),
);
...
export const updateDonorProfile = wrap(async (req, res) => {
  ...
  const updated = await updateDoc(COLLECTIONS.donors, req.user.donor_id, patch);
